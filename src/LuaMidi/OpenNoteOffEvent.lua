...
local OpenNoteOffEvent = {}

function OpenNoteOffEvent.new(fields)
   ...
   self.build_data = function()
      ...
      local note_off = NoteOffEvent.new({data = data})
      self.data = Util.table_concat(self.data, note_off.data)
      
   end
   self.build_data()
   return setmetatable(self, { __index = OpenNoteOffEvent })
end

-------------------------------------------------
-- Prints event's data in a human-friendly style
-------------------------------------------------
function OpenNoteOffEvent:print()
   ...
   print("\nClass / Type:\tOpenNoteOffEvent / '"..self.type.."'")
   print(str)
end

function OpenNoteOffEvent:set_pitch(pitch)
...
function OpenNoteOffEvent:set_velocity(velocity)
...
function OpenNoteOffEvent:set_channel(channel)
...
function OpenNoteOffEvent:set_timestamp(timestamp)
   assert(type(self.timestamp) == 'number' and self.timestamp >= 0, "'timestamp' must be an positive integer representing the explicit number of ticks")
...
function OpenNoteOffEvent:get_pitch()
   return self.pitch
end

function OpenNoteOffEvent:get_velocity()
   return self.velocity
end

function OpenNoteOffEvent:get_channel()
   return self.channel
end

function OpenNoteOffEvent:get_timestamp()
   return self.timestamp
end

return OpenNoteOffEvent
